...
SELECT title,rating FROM series JOIN reviews ON series.id=reviews.series_id;
SELECT genre, AVG(rating) AS avg_rating FROM series  JOIN reviews ON reviews.series_id=series.id GROUP BY genre;
SELECT 
    first_name,
    last_name,
...
FROM
    reviewers
       LEFT JOIN
    reviews ON reviews.reviewer_id = reviewers.id GROUP BY first_name,last_name;
    SELECT title,rating, CONCAT(first_name,' ',last_name) AS Reviewer FROM series JOIN reviews ON reviews.reviewer_id=series.id JOIN reviewers ON reviewers.id=reviews.reviewer_id; 
